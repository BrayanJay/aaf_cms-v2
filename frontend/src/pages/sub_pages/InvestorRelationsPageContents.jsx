...
function InvestorRelationsPageContents() {
  ...
  const handleUpload = async (num) => {
    ...
    formData.append("file_directory", "media/investorRelations"); // Custom File Path
  ...
  const fetchUser = async () => {
    try {
      const token = localStorage.getItem('token')
      const response = await axios.get('http://localhost:3000/auth/investorrelationspagecontents', {
        headers: {
          "Authorization" : `Bearer ${token}`
        }
      })
  ...
  return (
    <div className="flex justify-center container py-20">
      <div className="w-full m-10 px-20">
        <form>
          <label className="text-blue-800 font-medium">Investor Relations Page Carousel</label>
